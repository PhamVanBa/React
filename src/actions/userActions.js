  ...
  try {
    if (groupFilterForm) {
      ...
      if (groupFilterForm.startDate != null) {
        startDateConvert =
          groupFilterForm.startDate.getDate() +
          "/" +
      ...
      if (groupFilterForm.endDate != null) {
        endDateConvert =
          groupFilterForm.endDate.getDate() +
          "/" +
      ...
      let url =
        "http://localhost:8888/api/groups/paging?" +
        "pageNumber=" +
        groupFilterForm.pageNumber +
        "&size=" +
        groupFilterForm.pageSize +
        "&sort=" +
        groupFilterForm.sort +
        "&type=" +
        groupFilterForm.type +
        "&startDate=" +
        startDateConvert +
        "&endDate=" +
        endDateConvert;
...
const deleteGroupAll = (groupId) => async (dispath) => {
  dispath({
    type: constants.DELETE_GROUP_SELECTED_ROWS_REUEST,
  });
  ...
  try {
    ...
    dispath({
      type: constants.DELETE_GROUP_SELECTED_ROWS_SUCCESS,
      payload: response.data,
    });
  } catch (error) {
    dispath({
      type: constants.DELETE_GROUP_SELECTED_ROWS_FAIL,
      payload: {
        statusCode: error.response.status,
        message: "Create group fail!",
      },
